...
public class LinuxSocket implements Socket {
  ...
  public LinuxSocket(int domain, int type, int protocol) {
  ...
  public Socket connect(SocketAddress address) {
    try (Scope scope = Scope.globalScope().fork()) {
      if (address instanceof InetSocketAddress) {
        ...
        in.get().sin_family$set((short) socketAddress.family());
        ...
        Pointer<in6_addr.anon$in6_u> anon$in6_u =
            scope.allocate(LayoutType.ofStruct(in6_addr.anon$in6_u.class));
        ...
        in6_addr.get().__in6_u$set(anon$in6_u.get());

        in6.get().sin6_family$set((short) socketAddress.family());
        ...
        if (NativeSocket.connect(fd, sockaddr, (int) in6.type().bytesSize()) == 0) {
          Array<Byte> byteArray = in6.get().sin6_addr$get().__in6_u$get().__u6_addr8$get();
  ...
  public Socket accept() {
    try (Scope scope = Scope.globalScope().fork()) {
      ...
      if (storage.get().ss_family$get() == AddressFamily.AF_INET()) {
        ...
        Array<Byte> data = in6.get().sin6_addr$get().__in6_u$get().__u6_addr8$get();
